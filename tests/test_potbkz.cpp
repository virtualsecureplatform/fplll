...
int main()
{
  ...
  const int status = bkz_reduction(reduced, transform, 3, BKZ_POT_LLL, FT_DOUBLE);
  if (status != RED_SUCCESS)
  {
    cerr << "PotBKZ reduction failed: " << get_red_status_str(status) << endl;
  ...
  {
    cerr << "PotBKZ output is not PotLLL reduced" << endl;
    FP_NR<mpfr_t>::set_prec(old_prec);
    return 1;
  }
  if (!has_expected_transform(original, transform, reduced))
  {
    cerr << "PotBKZ transformation matrix does not reproduce the reduced basis" << endl;
    FP_NR<mpfr_t>::set_prec(old_prec);
    return 1;
  }
